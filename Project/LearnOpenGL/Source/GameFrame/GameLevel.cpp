#include "GameLevel.h"
#include "Managers/ResourceManager.h"
#include <fstream>
#include <sstream>


void GameLevel::Init(const vector<vector<GLuint>> & tileData, GLfloat temWidth, GLfloat temHeight)
{
	
	GLuint height = tileData.size();
	GLuint width = tileData[0].size();
	GLfloat unit_width = temWidth / static_cast<GLfloat>(width), unit_height = temHeight / height;
	
	for (GLuint y = 0; y < height; ++y)
	{
		for (GLuint x = 0; x < width; ++x)
		{
			GameObject * temObject = nullptr;
			if (tileData[y][x] == 1)
			{
				glm::vec2 pos(unit_width * x, unit_height * y);
				glm::vec2 size(unit_width, unit_height);
				temObject = new GameObject(RS_M->LoadTexture2D("block_solid", "texture/block_solid.png"), pos, size, vec3(0.8f, 0.8f, 0.7f));
				temObject->IsSolid = GL_TRUE;
			}
			else if (tileData[y][x] > 1)
			{
				glm::vec3 color = glm::vec3(1.0f);
				if (tileData[y][x] == 2)
					color = glm::vec3(0.2f, 0.6f, 1.0f);
				else if (tileData[y][x] == 3)
					color = glm::vec3(0.0f, 0.7f, 0.0f);
				else if (tileData[y][x] == 4)
					color = glm::vec3(0.8f, 0.8f, 0.4f);
				else if (tileData[y][x] == 5)
					color = glm::vec3(1.0f, 0.5f, 0.0f);

				glm::vec2 pos(unit_width * x, unit_height * y);
				glm::vec2 size(unit_width, unit_height);
				temObject = new GameObject(RS_M->LoadTexture2D("block", "texture/block.png"), pos, size, color);
			}
			if (temObject)
			{
				this->Bricks.push_back(temObject);
			}
		}
	}
}
void GameLevel::Load(const GLchar * file, GLfloat width, GLfloat height)
{
	Bricks.clear();
	
	GLuint tileCode;
	std::string line;
	std::ifstream fstream(file);
	std::vector<std::vector<GLuint>> tileData;
	if (fstream)
	{
		while (std::getline(fstream, line))
		{
			std::istringstream sstream(line);
			std::vector<GLuint> row;
			while (sstream >> tileCode)
			{
				row.push_back(tileCode);
			}	
			tileData.push_back(row);
		}
		if (tileData.size() > 0)
		{
			Init(tileData, width, height);
		}
			
	}
}
void GameLevel::Draw(Sprite * sprite)
{
	for (GameObject * tile : Bricks)
	{
		if (!tile->IsDestroyed)
		{
			tile->Draw(sprite);
		}	
	}
		
}
GLboolean GameLevel::IsComplete()
{

	return false;
}